 ...
 *    authByBindOnly {Boolean}
 *        Optional, to use only given credentails for user authentication - no
 *        admin login and search catalogue is used
 *    bindDn {String}
...
function LdapAuth(opts) {
  ...
  this.clientOpts = {
    ...
    authByBindOnly: opts.authByBindOnly,
  ...
  if (!this.clientOpts.authByBindOnly) {
...
LdapAuth.prototype.authenticate = function (username, password, callback) {
  ...
  if (self.opts.authByBindOnly) {
    // 1. Bind with given credentials and return true if successful
